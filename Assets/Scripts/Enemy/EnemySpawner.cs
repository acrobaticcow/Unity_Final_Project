...
public class EnemySpawner : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            EnemyObjectPools.Add(
                i,
                ObjectPool.CreateInstance(Enemies[i].Prefab, SpawnCount, PoolLayer)
            );
        }
    }
        ...
        if (poolObject != null)
        {
            ...
            if (
                NavMesh.SamplePosition(
                    triangulation.vertices[navMeshIndex],
                    out NavMeshHit hit,
                    2f,
                    -1
                )
            ...
            {
                Debug.LogError(
                    $"Unable to place enemy on navMesh, tried to use {triangulation.vertices[navMeshIndex]}"
                );
            }
        }
        else
