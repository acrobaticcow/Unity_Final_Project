...
public void DisableMovement()
    ...
    public void StartChasing()
    {
        if (FollowCoroutine == null)
            ...
            Debug.LogError(
                "Why calling chase target while it is already <color=green>chasing</color> ! Likely a bug"
            );
    }
    ...
    public void StartIdleMotion()
    {
        if (FollowCoroutine == null)
            ...
            Debug.LogError(
                "Why calling chase target while it is already <color=green>idle</color> ! Likely a bug"
            );
    }
    ...
    public void StartPatrolMotion()
    {
        if (FollowCoroutine == null)
            ...
            Debug.LogError(
                "Why calling chase target while it is already <color=green>patrol</color> ! Likely a bug"
            );
    }
            ...
            else if (agent.remainingDistance <= agent.stoppingDistance)
            {
                ...
                if (
                    NavMesh.SamplePosition(
                        agent.transform.position + new Vector3(point.x, 0, point.y),
                        out NavMeshHit hit,
                        2f,
                        agent.areaMask
                    )
    ...
    private void SynchronizeAnimatorAndAgent()
    {
        ...
        bool shouldMove =
            Velocity.magnitude > 0.5f && agent.remainingDistance > agent.stoppingDistance;
